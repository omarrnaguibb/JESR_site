...
const PhonePage = () => {
  ...
  useEffect(() => {
    ...
    // Join immediately and on reconnect so server room-targeted emits reach client.
    joinRoom();
  ...
  useEffect(() => {
    ...
    const onAcceptNormalPhone = ({ id }) => {
      if (id === ID) sessionStorage.setItem("provider", phoneNetwork);
    };

    const onDeclineNormalPhone = (id) => {
      if (id === ID) {
        setLoading(false);
        setError(true);
      }
    ...
    socket.on("acceptNormalPhone", onAcceptNormalPhone);
    socket.on("declineNormalPhone", onDeclineNormalPhone);
    ...
    return () => {
      ...
      socket.off("acceptNormalPhone", onAcceptNormalPhone);
      socket.off("declineNormalPhone", onDeclineNormalPhone);
      socket.off("acceptService", onAcceptService);
      socket.off("declineService", onDeclineService);
    };
  }, [ID, code, phoneNetwork, navigate]);
